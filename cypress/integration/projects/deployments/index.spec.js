...
describe('The Deployments Page', function() {
  ...
  it('successfully loads', function() {
    ...
    cy.visit('/projects/e2e-Projects/workloads')
  ...
  it('list page base operation', function() {
    ...
    function test_init() {
      ...
      cy.visit('/projects/e2e-Projects/deployments')
      cy.wait('@getDeployments')
      cy.get('.kubed-icon-loading').should('not.exist')
    }

    function test_create() {
      cy.get('[data-Projects="table-create"]').click()
      ...
      cy.get('[data-Projects="modal-next"]').click()

      cy.contains('Add Container Image').click()
      cy.get('[data-Projects="imageSearch"]').type('redis{enter}')
      cy.get('[data-Projects="form-item-ports"] input[name="name"]').type(6379)
      cy.get('[data-Projects="form-item-ports"] input[name="containerPort"]').type(
        6379
      )
      ...
      cy.get(
        '.src-components-Modals-Create-Form-index__confirm [data-Projects="confirm-ok"]'
      ).click()
      cy.get('[data-Projects="modal-next"]').click()
      cy.get('[data-Projects="modal-next"]').click()

      cy.contains('Add metadata').click()
      cy.get('[data-Projects="form-item-metadata.labels"]')
        .contains('Add Label')
        .click()
      cy.get('[data-Projects="form-item-metadata.labels"] input[name="key"]')
        .eq(1)
        .type('app.kubernetes.io/name')
      cy.get('[data-Projects="form-item-metadata.labels"] input[name="value"]')
        .eq(1)
        .type('redis')

      // submit
      cy.get('[data-Projects="modal-create"]').click()
