...
describe('The Deployment Detail Page', function() {
  ...
  it('successfully loads', function() {
    ...
    cy.get('[data-Projects="detail-title"]').contains(formData.metadata.name)
    cy.get('[data-Projects="detail-labels"]').contains('app')
    cy.get('[data-Projects="detail-labels"]').contains(formData.metadata.name)
  })
  ...
  it('detail page base operation', function() {
    ...
    function test_edit() {
      cy.log('Projects edit')
      cy.get('[data-Projects="detail-edit"]').click()
      cy.get('[name="metadata.annotations[\'kubesphere.io/alias-name\']"]')
        .clear()
        .type('redis_test')

      cy.get('[data-Projects="modal-ok"]').click()
    ...
    function test_redeploy() {
      cy.log('Projects redeploy')
      cy.get('[data-Projects="detail-more"]').click()
      cy.get('[data-Projects="detail-redeploy"]').click()
    ...
    function test_editYaml() {
      cy.log('Projects edit yaml')

      cy.get('[data-Projects="detail-more"]').click()
      cy.get('[data-Projects="detail-editYaml"]').click()
  ...
  it('detail page resource status', function() {
    cy.visit(
      `/projects/e2e-test/deployments/${formData.metadata.name}/resource-status`
    )
    cy.wait('@getDeployment')
    cy.wait('@getRules')
    cy.get('.kubed-icon-loading').should('not.exist')
    ...
    // check ports
    cy.get('[data-Projects="panel-ports"]').contains('6379')
    // check pod item
    cy.get('[data-Projects="panel-pods"] .src-components-Cards-Pods-index__item')
      .its('length')
      .should('be.gt', 0)
  ...
  it('detail page revision', function() {
    ...
    cy.get('[data-Projects="modal-close"]').click()
    ...
    cy.get('[data-Projects="detail-more"]').click()
    cy.get('[data-Projects="detail-rollBack"]').click()

    cy.get('[data-Projects="form-item-revision"] .select-control').click()
    cy.get('[data-Projects="form-item-revision"] .select-option')
      .first()
      .click()
  ...
  it('detail page monitoring', function() {
    cy.visit(
      `/projects/e2e-test/deployments/${formData.metadata.name}/monitors`
    )
    cy.wait('@getDeployment')
    cy.wait('@getMetrics')
    cy.get('.kubed-icon-loading').should('not.exist')
  ...
  it('detail page edit config template', function() {
    ...
    cy.get('[data-Projects="detail-more"]').click()
    cy.get('[data-Projects="detail-editConfigTemplate"]').click()
    cy.get('[data-Projects="modal-ok"]').click()
  })
  ...
  it('detail page hpa', function() {
    ...
    cy.get('[data-Projects="detail-more"]').click()
    cy.get('[data-Projects="detail-editHpa"]').click()
    cy.get('[data-Projects="modal-ok"]').click()
  })
