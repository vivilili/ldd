...
describe('The Projects Page', function() {
  before(function() {
    ...
    cy.request({
      ...
      // url: `kapis/consortiumcontroller.kubesphere.io/v1alpha1/initFabric/${formData.metadata.name}`,
      ...
      if (!resp.body.exist) {
        cy.request({
          ...
          // url:'kapis/consortiumcontroller.kubesphere.io/v1alpha1/initFabric',
          body: formData,
        })
      } else {
        cy.request({
  ...
  it('list page base operation', function() {
    ...
    // search
    {
      cy.get('[data-Projects="search"] > input').type(`kubesphere-system{enter}`)
    ...
    // assign workspace
    {
      ...
      cy.get(`[data-row-key="${formData.metadata.name}"]`).contains('e2e-Projects')
      cy.get(
