...
export default class ProjectStore extends Base {
  ...
  createAlliance = async ({
    ...
    const { metadata } = data
    localStorage.setItem('chain_name',JSON.stringify(metadata.chain_name))
    const chain_name = JSON.parse(localStorage.getItem("chain_name"))   
    const newParams = {
      "namespace": metadata.namespace,
      "orderer_num":(metadata.orderer_num)* 1,
      "orderer_ports":(metadata.orderer_ports).split(",").map(Number),
      "chain_name":chain_name,
      "peer_num":(metadata.peer_num)* 1,
      "peer_ports":(metadata.peer_ports).split(",").map(Number),
      "init_org_name":metadata.init_org_name,
      "consortium_description":metadata.consortium_description,
      "consortium_name_display":metadata.consortium_name_display,
      "consortium_framework":metadata.consortium_framework,
      "master_host_ip":metadata.master_host_ip,
      "passwords" :(metadata.passwords).split(",")
  }
    // console.log(data);
    // console.log(newParams);
    const result = await to(request.post('/kapis/consortiumcontroller.kubesphere.io/v1alpha1/initFabric', newParams))
